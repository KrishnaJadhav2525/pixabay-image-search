...
export default function ImageModal({ image, onClose }) {
  ...
  return (
    <div id="image-modal-overlay" className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      ...
      <div className="relative z-10 flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl glass animate-scale-in lg:flex-row" onClick={(e) => e.stopPropagation()}>
        <button id="modal-close-button" onClick={onClose} className="absolute right-3 top-3 z-20 flex h-9 w-9 items-center justify-center rounded-full bg-black/40 text-white/70 transition-colors hover:bg-black/60 hover:text-white cursor-pointer" aria-label="Close modal">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
          </svg>
        </button>
        <div className="flex flex-1 items-center justify-center bg-dark-950/50 p-2 lg:p-4">
          <img src={image.largeImageURL} alt={image.tags} className="max-h-[50vh] w-full rounded-xl object-contain lg:max-h-[80vh]" />
        </div>
        <div className="flex w-full flex-col gap-5 p-6 lg:w-80 lg:overflow-y-auto">
          ...
          <div className="flex flex-wrap gap-1.5">
            {image.tags?.split(', ').map((tag) => (
              <span key={tag} className="rounded-full bg-dark-700 px-2.5 py-1 text-xs text-dark-300">#{tag}</span>
            ))}
          </div>
          ...
          <div className="rounded-xl bg-dark-800 p-3">
            ...
            <p className="text-sm font-semibold text-dark-100">{image.imageWidth} x {image.imageHeight}</p>
          </div>
        </div>
      </div>
    </div>
